...
class RevealBase extends React.Component {
  ...
  getDimensionValue() {
    return this.el.offsetHeight + Number.parseInt(window.getComputedStyle(this.el, null).getPropertyValue('margin-top'), 10) + Number.parseInt(window.getComputedStyle(this.el, null).getPropertyValue('margin-bottom'), 10)
  }
  ...
  animate(props) {
    ...
    const inOut = props[leaving ? 'outEffect' : 'inEffect']
  ...
  cascade(children) {
    ...
    if (this.props.collapse) {
      total = Number.parseInt(this.state.style.animationDuration, 10)
      duration = total / 2
    }
  ...
  inViewport(props) {
    ...
    const delta = window.pageYOffset/* - props.margin */ - RevealBase.getTop(this.el)
    const tail = Math.min(h, window.innerHeight) * (globalHide ? props.fraction : 0)
